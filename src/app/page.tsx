        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-xs font-medium tracking-widest text-muted-foreground uppercase">
              Prompt library · {PROMPTS.length} curated examples
            </p>
            <h1 className="mt-2 max-w-xl text-3xl font-medium text-balance tracking-tight">
              Collect prompts worth
              <br />
              keeping.
            </h1>
            <p className="mt-3 max-w-lg text-sm text-pretty leading-relaxed text-muted-foreground">
              Each card pairs an image with the exact prompt that made it — tagged by
              model and style so you can find what fits your tool and remix it.
            </p>
          </div>
          <div className="flex shrink-0 items-center gap-2 text-xs text-muted-foreground">
            <span className="rounded-full border bg-card px-3 py-1.5 tabular-nums">
              Open the card → copy in one click
            </span>
          </div>
        </div>
      </section>
