  ...
  return (
    <div className="flex flex-col gap-5">
          ...
          {topRated.map((s, i) => {
            ...
            return (
              <li key={s.id}>
                ...
                >
                  <span className="flex w-5 shrink-0 justify-center text-sm font-bold tabular-nums text-muted-foreground">
                    {i + 1}
                  </span>
                  <span
                    className={cn(
                  ...
                  <span className="shrink-0 text-sm tabular-nums font-semibold text-foreground">
                    {m.rating.toFixed(1)}
                  </span>
                </button>
              </li>
      ...
      <div className="rounded-2xl bg-gradient-to-br from-sky-50 to-indigo-100 p-4">
        <p className="text-base font-semibold text-indigo-950">Follow the marketplace</p>
        <p className="mt-1 text-sm leading-relaxed text-indigo-900/70">
          New servers land daily. Get the feed delivered, upvote-first.
        </p>
        <button
          type="button"
          className="mt-3 inline-flex w-full items-center justify-center gap-1.5 rounded-full bg-indigo-600 px-3 py-2 text-base font-medium text-white hover:bg-indigo-500"
        >
